...
#get path for data
pwd = os.getcwd()
#dataset_path = pwd + '\\wbdata_hdr_april.csv'
dataset_path = pwd + '\\wbdata_hdr_latest.csv'
...
#calculate months
month1 = (today+relativedelta(months=-4)).strftime('%m-%Y') #4 months ago
month2 = (today+relativedelta(months=-3)).strftime('%m-%Y') #3 months ago
month3 = (today+relativedelta(months=-2)).strftime('%m-%Y') #last month -2
month4 = (today+relativedelta(months=-1)).strftime('%m-%Y') #last month
...
newDateFormat['diaryEntryDate'] = pd.to_datetime(newDateFormat['diaryEntryDate']) #convert to datetime
...
def top5_low5(df):
    ...
    #get dates
    max_date = max(dataset_melted['entryMonth'])
    last_month = monthdelta(datetime.strptime(max_date, '%m-%Y'), -1).strftime('%m-%Y')
    prev_month = monthdelta(datetime.strptime(max_date, '%m-%Y'), -2).strftime('%m-%Y')
...
def var_heatmap(df):
    ...
    #dend = shc.dendrogram(shc.linkage(core_data, method='ward'))
...
def monthly_boxplot(df, column):
    ...
    #get dates
    latest_date = max(df['entryMonth'])
    month1b = monthdelta(datetime.strptime(latest_date, '%m-%Y'), -4).strftime('%m-%Y')
    month2b = monthdelta(datetime.strptime(latest_date, '%m-%Y'), -3).strftime('%m-%Y')
    month3b = monthdelta(datetime.strptime(latest_date, '%m-%Y'), -2).strftime('%m-%Y')
    month4b = monthdelta(datetime.strptime(latest_date, '%m-%Y'), -1).strftime('%m-%Y')
